...
class CD:
    ...
    """
    # DONE Modify CD class as required
    # -- Constructor -- #
    ...
    def rmv_track(self, track_id: int) -> None:
        ...
        for item in self.__tracks:
            if item.position == track_id:
                self.__tracks.remove(item)
            else:
                continue
    ...
    def get_long_record(self) -> str:
        ...
        """
        result = 'CD_ID --- CD TITLE --- CD ARTIST\n------------------------------\n'
        result += self.get_record() + '\n'
        result += 'TRACK --- TITLE --- (LENGTH)\n------------------------------\n'
